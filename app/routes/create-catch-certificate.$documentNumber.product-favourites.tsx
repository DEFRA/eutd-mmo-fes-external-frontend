...
export const action: ActionFunction = async ({ request, params }): Promise<Response | ErrorResponse> =>
  await manageFavouritesAction(request, `/create-catch-certificate/${params.documentNumber}/product-favourites`);

const ManageFavourites = () => {
  const { documentNumber } = useLoaderData<ManageFavouritesProps>();
  return <ManageFavouritesHomeComponent backUrl={`/create-catch-certificate/${documentNumber}/upload-file`} />;
};
