...
This skill runs inside the **working framework** in [copilot-instructions.md](../../copilot-instructions.md) §4 — it does **not** restate or fork it. Follow the standards precedence (DEFRA > GDS > community, where GDS covers the GOV.UK Design System and WCAG 2.2 AA accessibility) and honour the triage split: framework-**trivial** work takes the light Read → Implement → Test → Summarise fast-path; **Standard** work (a normal feature/page/route/fix with no new architecture, auth, session/cookie or security surface) uses a lightweight inline plan plus approval; **Complex** work (loaders/actions, CSRF, auth/session/cookie architecture, server-only `.server.ts` architecture, a new external integration, a security surface) needs a full Planner-authored plan and approval first. For anything version- or policy-sensitive, use the [deep-research-defra-alignment](../deep-research-defra-alignment/SKILL.md) skill for a single risk-scoped pass and cite sources. Validate with this repo's real flow — `npm run lint`, the instrumented Cypress + MSW flow (`npm run pre:test:start` → `npm run :test:start` → `npm run :test:all`) and `npm run build` — never Jest.
