...
- **Planner - External Frontend** runs this single research pass for **Complex** work (unfamiliar APIs,
  accessibility, bilingual coverage, CSRF, auth/session, security, policy) and cites sources directly in its
  plan.
- **Developer - External Frontend** runs this same single pass for **Standard** work (or when invoked
  without a plan) as its own Research (§4.2) stage.

## When to use
- **Research (§4.2), single pass:** an unfamiliar API, library, pattern, accessibility rule, or policy point
  is genuinely uncertain — for the Planner (Complex work) or the Developer (Standard work).
