...
describe("DirectLanding page errors when javascript is enabled", () => {
  ...
  it("should correctly parse float strings using Number.parseFloat()", () => {
    waitForHydration();
    ...
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("12.345", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("7.655", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();

    cy.contains(".govuk-table__cell", "Total export weight").next(".govuk-table__cell").should("have.text", "20.00kg");
  });

  it("should handle Number.parseFloat() with leading/trailing whitespace", () => {
    waitForHydration();

    cy.contains(".govuk-table__cell", "Total export weight").should("be.visible");

    // Test with spaces (Note: input fields typically trim values, but testing the concept)
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("10.5", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("9.5", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();

    cy.contains(".govuk-table__cell", "Total export weight").next(".govuk-table__cell").should("have.text", "20.00kg");
  });

  it("should handle Number.parseFloat() with very small decimal values", () => {
    waitForHydration();
    ...
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("0.001", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("0.009", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();

    cy.contains(".govuk-table__cell", "Total export weight").next(".govuk-table__cell").should("have.text", "0.01kg");
  });

  it("should handle Number.parseFloat() with integer strings", () => {
    waitForHydration();

    cy.contains(".govuk-table__cell", "Total export weight").should("be.visible");

    // Test with integer strings (parseFloat should work fine)
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    ...
    cy.contains(".govuk-table__cell", "Total export weight").next(".govuk-table__cell").should("have.text", "150.00kg");
  });

  it("should handle Number.parseFloat() returning NaN for invalid input", () => {
    waitForHydration();

    cy.contains(".govuk-table__cell", "Total export weight").should("be.visible");

    // Test with invalid input that parseFloat would return NaN
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("invalid", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    // Set second weight to valid number
    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("25.5", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();

    // Total should only include valid parsed values, excluding NaN
    cy.contains(".govuk-table__cell", "Total export weight").next(".govuk-table__cell").should("have.text", "25.50kg");
  });

  it("should handle Number.parseFloat() with scientific notation", () => {
    waitForHydration();

    cy.contains(".govuk-table__cell", "Total export weight").should("be.visible");

    // Test with scientific notation (parseFloat handles this)
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("1e2", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("5e1", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();

    cy.contains(".govuk-table__cell", "Total export weight").next(".govuk-table__cell").should("have.text", "150.00kg");
  });

  it("should handle Number.parseFloat() with negative values", () => {
    waitForHydration();

    cy.contains(".govuk-table__cell", "Total export weight").should("be.visible");

    // Test with negative values (parseFloat handles negatives)
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("10", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("5", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();
  ...
  it("should handle Number.parseFloat() with multiple decimal points gracefully", () => {
    waitForHydration();

    cy.contains(".govuk-table__cell", "Total export weight").should("be.visible");

    // parseFloat would parse "12.34.56" as 12.34 and stop at second decimal
    cy.get('input[id="weights.0.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.0.exportWeight"]').type("12.34", { force: true });
    cy.get('input[id="weights.0.exportWeight"]').blur();

    cy.get('input[id="weights.1.exportWeight"]').clear({ force: true });
    cy.get('input[id="weights.1.exportWeight"]').type("7.66", { force: true });
    cy.get('input[id="weights.1.exportWeight"]').blur();
