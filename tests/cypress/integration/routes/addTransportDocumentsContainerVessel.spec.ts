...
const containerVesselPageUrl = `create-catch-certificate/${documentNumber}/add-additional-transport-documents-container-vessel/0`;

describe("Add Transportation Documents Container Vessel", () => {
  it("should render container vessel transport details page", () => {
    ...
    cy.contains("a", /^Back$/)
      .should("be.visible")
      .should("have.attr", "href", `${certificateUrl}/add-transportation-details-container-vessel/0`);
    cy.get(".govuk-fieldset__heading").contains("Container vessel documents");
    cy.get("form").should(($form) => {
      ...
      expect(labels).to.deep.eq([
        "Container vessel document name (optional)",
  ...
  it("should display previously entered name and reference", () => {
    ...
    cy.get('label[for="documents.0.reference"]').should(
      "have.text",
      "Cyfeirnod y ddogfen llong cynwysyddion (dewisol)"
    );
  });

  it("should redirect user to forbidden page when saveTransportDocuments fails with a 403 error", () => {
    const testParams: ITestParams = {
      testCaseId: TestCaseId.ContainerVesselTransportDocumentsFailsWith403,
    };
