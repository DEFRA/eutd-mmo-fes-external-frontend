...
const pageUrl = "create-processing-statement/GBR-2022-PS-F71D98A30/copy-this-processing-statement";

describe("Copy this processing statement address page: Allowed", () => {
  beforeEach(() => {
  ...
  it("should render back button when on click should to navigate to processing statement dashboard page", () => {
    cy.contains("a", /^Back$/)
      .should("be.visible")
  ...
  it("should render the  Create draft processing statement button", () => {
    cy.contains("button", "Create draft processing statement").should("be.visible");
  });
  ...
  it("should render the radio and check box copy options and labels ", () => {
    cy.get("form").should(($form) => {
      ...
      expect(labels).to.deep.eq([
        ...
        "I understand I must not reuse the same processing statement or data for catches that have already been exported",
      ]);
    });
  });
});
