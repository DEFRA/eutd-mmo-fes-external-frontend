...
const uploadFavouritesUrl = `/create-catch-certificate/${docummentNumber}/upload-favourites`;

describe("Upload favourites page", () => {
  beforeEach(() => {
  ...
  it("should render a back link", () => {
    cy.contains("a", /^Back$/)
      .should("be.visible")
      .should("have.attr", "href", `/create-catch-certificate/${docummentNumber}/upload-guidance`);
    cy.findByRole("link", { name: "Back" }).click({ force: true });
    cy.url().should("include", "/create-catch-certificate/GBR-2022-CC-012345678/upload-guidance");
  });
...
describe("Upload favourites page: when JavaScript is disabled", () => {
  beforeEach(() => {
    const testParams: ITestParams = {
      testCaseId: TestCaseId.ManageFavourites,
      args: ["catchCertificate"],
  ...
  it("should redirect to #add-state", () => {
    ...
    cy.get("[data-testid='add-species']").click({ force: true });
    cy.url().should("include", "/upload-favourites#add-state");
  });
});

describe("Upload favourites page with errors", () => {
  beforeEach(() => {
    const testParams: ITestParams = {
...
describe("Upload favourites page: handling data", () => {
...
describe("Upload favourites page: validating inputs", () => {
...
describe("Upload favourites page: removing a product", () => {
  it("should remove a product and stay on the upload favourites page", () => {
    const testParams = {
      testCaseId: TestCaseId.ManageFavourites,
    };
